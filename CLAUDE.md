...
1. **Deploy prod** — SSH na VPS, setar `AUDIT_SYNC_ENABLED=true` + `DATABASE_URL=<audit_writer URL>`, restart do container `pje-dashboard`. Validado localmente via docker-compose + Playwright.
   - ⚠️ **`deploy.yml` falha em 0s em TODO push para `master` — isto é ESPERADO, não é regressão.** O gate fail-fast `Validate required secrets` (Task 1.1) aborta porque os secrets de deploy (`VPS_HOST`, `VPS_SSH_KEY`, `VPS_USER`, `MNI_*`, `REDIS_PASSWORD`, `DASHBOARD_API_KEY`) ainda não estão configurados no repo. Fica vermelho até este item ser feito. **Não investigar como bug.** O gate verde a checar é o `ci.yml` (lint + pytest), separado.
