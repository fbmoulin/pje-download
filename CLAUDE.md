...
6. **Achados de code-review sobre #46/#47 (já em produção)** — 2026-09-25, dois subagentes `/code-review` rodaram (por engano) contra uma ref local `master` desatualizada, revisando o diff inteiro de #46+#47 como se fosse trabalho pendente. Não é: já está mesclado e deployado. Nenhuma ação tomada nesta sessão (mudar esse código = push em `master` = redeploy, fora do escopo do que foi pedido). Guardado aqui como tarefa futura, não urgente:
   - **Correção/comportamento removido (os que mais importam):**
     - `worker.py` `_download_document_api` — o branch `except Exception` genérico (falhas que não são `OSError`, ex. timeout do Playwright) nunca chama `_audit_document_saved`, quebrando a garantia de trilha de auditoria CNJ 615/2025 para essa classe de falha. Resposta HTTP não-200 também nunca é auditada (pré-existente, mas agora inconsistente com os caminhos vizinhos que passaram a auditar).
     - `worker.py` — F4 (`_ensure_browser()` lazy em modo MNI) reabre um branch que antes era inalcançável (`session_lost` via `self.page.url`), e esse branch ainda chama o `invalidate_session()` de exclusão incondicional em vez do `_close_browser()` que F7 introduziu justamente para navegador-helper em modo MNI — pode forçar um re-login desnecessário.
     - `gdrive_downloader.py` `extract_folder_id` — troca de `re.search` para `fullmatch` rejeita URLs antes válidas com segmento extra (`.../folders/ID/view`); exigência nova de `scheme == "https"` rejeita links `http://` que `extract_gdrive_link_from_pje` (mesmo arquivo) ainda extrai sem filtro — inconsistência produtor/consumidor no mesmo módulo (sem call site em produção ainda, confirmado por grep).
     - `mni_client.py` `verify_credentials()` — classifica por substring hardcoded (`"Processo não encontrado"`, `"Acesso negado"`/`"Unauthorized"`); texto de erro SOAP diferente por tribunal pode dar falso "inconclusive" (credencial boa) ou falso "valid" (credencial ruim).
     - `worker.py` `_close_browser` nunca reseta `self.session_started_at` (baixa severidade — `self.page` fica `None` depois, então o branch de expiração é pulado na próxima checagem, mas é um invariante que o `invalidate_session()` antigo sempre mantinha).
   - **Qualidade (reuse/simplification/efficiency/altitude, menor prioridade):** duplicação do builder `AuditEntry(event_type="document_saved", ...)` entre `worker.py` e `mni_client.py`; `audit_sync.py` faz 3 varreduras de diretório redundantes por tick (`_tick`/`_pending_bytes`/`_oldest_pending_event_ts`) e roda I/O síncrono de disco no mesmo event loop do aiohttp sem `asyncio.to_thread` (convenção do próprio CLAUDE.md, não seguida aqui); `gdrive_downloader.py` só carrega `resourcekey` em 2 das 3 estratégias de download (gdown ignora mesmo, então só Playwright se beneficia); `is_session_expired()` continua "cego" a modo — a distinção MNI-vs-browser vive só no call site, não no método.
