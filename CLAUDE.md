...
5. **zeep SSRF hardening (defense-in-depth)** — bump para `zeep==4.3.3` já feito (`23d5e8a`, fecha Dependabot alert #1 / GHSA-4cc2-g9w2-fhf6). Falta: passar `Settings(forbid_external=True)` no `Client(...)` em `mni_client.py:178`. **Não aplicar às cegas** — WSDLs do MNI/PJe frequentemente fazem `xsd:import` de schemas externos; ligar o flag pode quebrar o cliente com `ExternalReferenceForbidden`. Requer teste contra fetch de WSDL real. **⚠️ O bloqueador citado aqui NÃO existe mais** — dizia "hoje bloqueado por IP cloud", refutado desde 2026-07-18 (geo-bloqueio resolvido, ver item 1) e comprovado em 2026-07-25: `curl https://pje.tjes.jus.br/pje/intercomunicacao?wsdl` de IP BR devolve **HTTP 200, 34,5 KB**, e o WSDL do TJES tem **5 `xs:import`/`xs:include` mas ZERO `schemaLocation`** — ou seja, os imports são só de namespace e não disparam fetch externo, que é justamente o dado que faltava para avaliar `forbid_external=True`. ⚠️ Medido **só no TJES**: `TRIBUNAL_ENDPOINTS` tem 6 tribunais e um WSDL com `schemaLocation` externo quebraria o cliente com `ExternalReferenceForbidden`. Meça os outros 5 antes de ligar o flag para eles. Este item está desbloqueado. Ataque direto já mitigado: as URLs de WSDL são allowlist hardcoded `.jus.br` (`mni_client.py:46-51`). **Spec escrita 2026-09-25** (`docs/specs/2026-09-25-zeep-forbid-external.md`, PR #49): escopo reduzido a decisão do Felipe — hardening só em TJES por enquanto (`MNI_FORBID_EXTERNAL_TRIBUNALS`, config-driven), os outros 5 tribunais ficam como expansão futura (linha de config, não precisa de spec nova). Aguardando USER VALIDATION GATE (escopo do teste ao vivo pós-deploy) antes da implementação.
