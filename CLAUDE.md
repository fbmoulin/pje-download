...
5. ~~**zeep SSRF hardening (defense-in-depth)**~~ — **CÓDIGO PRONTO 2026-09-25** (spec `docs/specs/2026-09-25-zeep-forbid-external.md`, PR #49; ainda não mesclado/deployado neste momento). Bump para `zeep==4.3.3` já feito antes (`23d5e8a`, fecha Dependabot alert #1 / GHSA-4cc2-g9w2-fhf6). Escopo, por decisão do Felipe: **só TJES por enquanto**, não os 6 tribunais — `Settings(forbid_external=...)` agora é **por tribunal**, via `MNI_FORBID_EXTERNAL_TRIBUNALS` (`config.py`, default `{"TJES"}`, env-configurável), não mais um flag global. `mni_client.py:_get_client` (`~:213-217`) passa `settings=Settings(forbid_external=self.tribunal in MNI_FORBID_EXTERNAL_TRIBUNALS)` ao `Client(...)`. Confirmado empiricamente (não só suposto) contra o `zeep==4.3.3` instalado: a exceção é `zeep.exceptions.ExternalReferenceForbidden` direto — **não** `TransportError`/`XMLSyntaxError` como a spec original supôs — e ela dispara **antes** de qualquer tentativa de rede (`ImportResolver.resolve()` em `zeep/loader.py` checa `forbid_external` antes de chamar `transport.load()`). Prova de regressão em `tests/test_mni_client.py` (`TestForbidExternalSettings`, `TestForbidExternalSSRFFixture`) usa um fixture WSDL local (`tests/fixtures/wsdl_external_schema_location.wsdl`) com `schemaLocation` externo em RFC 5737 TEST-NET-1 (`192.0.2.1`, não `169.254.169.254` — esse é o IMDS real da Azure e poderia responder de verdade num runner Azure) — zeep/lxml reais, só `requests.Session.get` mockado, então prova o comportamento de verdade sem bater rede nem depender do provedor de nuvem do runner. Suite completa: 592 passed (+4 testes novos, 2 falhas pré-existentes não relacionadas — `chmod 0o444` não bloqueia root neste sandbox), `ruff` limpo na 0.14.14. **Falta:** Task 4 do spec — verificação ao vivo pós-deploy batendo o `/health` do worker (`checks["mni"]`) contra TJES em produção; não dá pra fazer antes do merge+deploy. Os outros 5 tribunais (`TJES_2G`, `TJBA`, `TJBA_2G`, `TJCE`, `TRT17`) seguem sem hardening — expansão futura é só medir `schemaLocation` de cada um a partir do `pje-vps` e adicionar à env var, sem PR de código novo. Ataque direto já mitigado independentemente: as URLs de WSDL são allowlist hardcoded `.jus.br` (`mni_client.py:46-51`).
6. ~~**Achados de code-review sobre #46/#47 (já em produção)**~~ — **4 dos 5 achados de correção CORRIGIDOS 2026-09-26** (branch `claude/full-analysis-bgj8hi`, ainda não mesclado neste momento). Origem: 2026-09-25, dois subagentes `/code-review` rodaram (por engano) contra uma ref local `master` desatualizada, revisando o diff inteiro de #46+#47 como se fosse trabalho pendente — já estava mesclado e deployado, mas os achados em si eram reais, então viraram este item de backlog em vez de descartados.
   - **Correção/comportamento removido — CORRIGIDOS:**
     - `worker.py` `_download_document_api` — o branch `except Exception` genérico e a resposta HTTP não-200 agora chamam `_audit_document_saved` também (antes só `OSError` auditava); os dois branches de exceção foram unificados (eram idênticos). +3 testes (`TestDocumentSavedAudit`).
     - `worker.py` — o branch `session_lost` no `except` externo de `download_process` agora usa `_close_browser()` (mantém o arquivo de sessão) em modo MNI, e só usa `invalidate_session()` (apaga o arquivo) em modo browser-primary — mesma distinção que o F7 já aplicava no branch de timeout operacional, só que este outro branch (reaberto pelo F4) tinha ficado de fora. +2 testes (`TestMniModeSessionLostDuringJobKeepsSessionFile`).
     - `gdrive_downloader.py` `extract_gdrive_link_from_pje` — os 3 pontos que retornavam a URL crua (regex no HTML, fallback via `href`, fallback via clique em documento) agora passam por `canonical_folder_url()` antes de retornar, igual ao fallback de iframe (que já fazia isso certo); se não passar (ex. `http://`), a extração continua procurando em vez de devolver um link que `download_gdrive_folder` rejeitaria mesmo assim. **`extract_folder_id` NÃO foi enfraquecido** — o `fullmatch`/exigência de `https` é o guard anti-SSRF documentado no próprio docstring, era a extração que precisava se alinhar a ele, não o contrário. +2 testes (`TestExtractGdriveLinkFromPje`).
     - `mni_client.py` `verify_credentials()` — o branch body-level (`sucesso=false`) agora distingue `not_found` (match estreito em `"não encontrado"`, cobre a variante real de produção do TJES "Processo de número X não encontrado!") de `mni_error` genérico; `_MNI_VERIFY_VALID_STATUSES` não inclui mais `mni_error` puro — uma rejeição com texto não reconhecido (por qualquer tribunal, por qualquer motivo, incluindo credencial ruim) agora reporta "inconclusive" em vez de "valid". +2 testes (renomeado 1, novo 1) em `TestVerifyCredentials`.
   - **Não corrigido, por decisão (baixa severidade, não um bug):** `worker.py` `_close_browser` continua sem resetar `self.session_started_at` — confirmado intencional (consistente com o F7 já existente: `self.page` fica `None`, então o branch de expiração é pulado na próxima checagem), não uma regressão.
   - **Qualidade (reuse/simplification/efficiency/altitude) — segue como backlog, menor prioridade, não tocado nesta rodada:** duplicação do builder `AuditEntry(event_type="document_saved", ...)` entre `worker.py` e `mni_client.py`; `audit_sync.py` faz 3 varreduras de diretório redundantes por tick (`_tick`/`_pending_bytes`/`_oldest_pending_event_ts`) e roda I/O síncrono de disco no mesmo event loop do aiohttp sem `asyncio.to_thread` (convenção do próprio CLAUDE.md, não seguida aqui); `gdrive_downloader.py` só carrega `resourcekey` em 2 das 3 estratégias de download (gdown ignora mesmo, então só Playwright se beneficia); `is_session_expired()` continua "cego" a modo — a distinção MNI-vs-browser vive só no call site, não no método.
