...
1. **Deploy prod** — SSH na VPS, setar `AUDIT_SYNC_ENABLED=true` + `DATABASE_URL=<audit_writer URL>`, restart do container `pje-dashboard`. Validado localmente via docker-compose + Playwright.
   - **Secrets 7/7 wired (2026-07-18).** 5 já existiam desde 2026-04-03 (`VPS_HOST`, `VPS_SSH_KEY`, `VPS_USER`, `MNI_USERNAME`, `MNI_PASSWORD`); os 2 que faltavam — `REDIS_PASSWORD` e `DASHBOARD_API_KEY` — foram gerados (`secrets.token_urlsafe(36)`, URL-safe p/ o `redis://:PASS@`) e setados via `gh secret set`. O gate `Validate required secrets` (Task 1.1) **agora passa**.
   - 🔴 **Blocker restante = host vivo.** `VPS_HOST` é de 2026-04-03 e o VPS antigo `191.252.204.250` está MORTO (Task 1.2 só removeu o IP dos docs, não atualizou o secret). Valor do secret é write-only — não dá p/ ler/verificar. **Antes de disparar o deploy:** provisionar/confirmar um VPS vivo (Hetzner é a escolha; precisa Docker + `docker compose` v2, `VPS_USER` no grupo docker, pub-key do `VPS_SSH_KEY` em `authorized_keys`, dirs `/opt/pje-download` + `/data/{downloads,audit}`), depois `printf '%s' 'HOST' | gh secret set VPS_HOST -R fbmoulin/pje-download`. Só o Felipe roda (valor não pode ir pro transcript).
   - Disparo: `gh workflow run deploy.yml` (path `workflow_dispatch`). 5 steps: rsync → escreve `.env` no VPS (a partir dos secrets) → `docker compose --profile worker up -d --build` + smoke health/queue → valida credenciais MNI.
   - ⚠️ **Enquanto o host não estiver vivo, `deploy.yml` continua vermelho — ESPERADO, não é bug.** Antes falhava em 0s no gate de secrets; agora passa o gate e falha adiante no `rsync` (host morto). O gate verde a acompanhar é o `ci.yml` (lint + pytest), separado.
