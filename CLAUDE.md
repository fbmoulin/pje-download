...
1. ~~**Deploy prod**~~ — ✅ **DONE 2026-07-18 — app NO AR num Hostinger VPS** (run verde `29644411566`, 25s, 5/5 steps). Host `2.24.126.161` (VM Hostinger `1700758`, KVM 2, hostname `srv1700758.hstgr.cloud`). 3 containers `Up (healthy)`: dashboard :8007, worker :8006, redis. MNI = `unhealthy` (bloqueio de IP do datacenter — esperado; ver follow-up MNI_PROXY abaixo). Dashboard acessível só por **túnel SSH** (`ssh -i ~/.ssh/pje_deploy -L 8007:localhost:8007 deploy@2.24.126.161`), 8007 fechado de fora pelo firewall. **`AUDIT_SYNC_ENABLED` segue off** (sink Railway opcional; audit JSON-L local funciona no volume).
   ...
   - **▶ Follow-up: MNI bloqueado por IP** — o passo `Validate MNI credentials` passa (construtor `MNIClient()` é lazy, só valida config), mas chamadas MNI reais vão falhar do datacenter → fallback Playwright. Para MNI real, usar `MNI_PROXY` (suportado em `mni_client.py:166`) apontando p/ um IP liberado.
